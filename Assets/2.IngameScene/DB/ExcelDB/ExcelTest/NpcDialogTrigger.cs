...
public class NpcDialogTrigger : MonoBehaviour
{
    ...
    [Header("↓출력을 원하는 다이얼로그 QuestID의 번호를 입력해주세요.")] 
    [SerializeField] private int questID; // 대화 출력을 원하는 퀘스트ID 선택
    ...
    {
        // 테스트 체크 [디버그]
        useDialogList = this.dialogDB.DialogSheet.Where(excelDB => excelDB.DialogID == questID).ToList();
    }

    private IEnumerator Start()
    {
        // setQuestID에 해당되는 Dialog 리스트를 매개변수로 보낸다. // Linq
        yield return new WaitUntil(() => DialogSystem.instance.UpdateDialog(this.dialogDB.DialogSheet.Where(excelDB => excelDB.DialogID == questID).ToList()));
    }
}
