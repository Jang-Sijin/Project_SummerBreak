...
public class QuestTrigger : MonoBehaviour
{
	...
	[SerializeField]
	private int setQuestID;

	[SerializeField]
	private List<QuestDialogDBEntity> runtimedataTable;
	...
	private void Start()
    {
		// setQuestID에 해당되는 Dialog 내용을 리스트에 저장. // Linq
		runtimedataTable = this.dialogDB.QuestSheet.Where(excelDB => excelDB.questID == setQuestID).ToList();


		foreach (var n in runtimedataTable)
        {
			print($"{n.questID}, {n.name}, {n.dialog}");
        }
    }
}
...
/*	[Header("숫자가 작아질수록 타이핑 속도가 빨라집니다.")]
	...
	[SerializeField]
	private List<DialogData> dialogs;                   // 현재 분기의 대사 목록 리스트
	...
	private void Awake()
	{
		for (int i = 0; i < dialogDB.Tutorial.Count; ++i)
		{
			if (dialogDB.Tutorial[i].branch == branch)
			{
				...
				dialogData.speakerIndex = dialogDB.Tutorial[i].speaker;
				dialogData.name = dialogDB.Tutorial[i].name;
				dialogData.dialogue = dialogDB.Tutorial[i].dialog;
...
{
	public Image spriteImage;       // 캐릭터 이미지 (청자/화자 알파값 제어)
