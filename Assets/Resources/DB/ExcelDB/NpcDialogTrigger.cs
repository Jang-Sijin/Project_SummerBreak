...
public class NpcDialogTrigger : MonoBehaviour
{
    ...
    private int _saveDialogID; // 퀘스트 시작하기 전 DialogID
    ...
    private void Start()
    {
        // 예시) DialogID: 11000일 때 11000~11099까지 리스트에 저장
        npcDialogList = dialogDB.DialogSheet.Where(excelDB => excelDB.DialogID % dialogID < 100).ToList();
        
        _saveDialogID = dialogID; // 시작 시 DB Sheet에서의 첫 다이얼로그ID를 저장한다.
    }
    ...
    { 
            ...
            (QuestSystem.instance.IsProgressQuest == true && QuestSystem.instance.ReturnProgressQuestDB().EndDialogID % dialogID < 100))
        {
            ...
            {
                yield return new WaitUntil(() =>
                    _isExitDialog = DialogSystem.instance.UpdateDialog(npcDialogList.Where(dialogDB => dialogDB.DialogID == _saveDialogID).ToList(), npcAnimator, npcVirtualCameraObj));
            }
        ...
        else // 2.일반 대화
        {
            ...
            yield return new WaitUntil(() =>
                _isExitDialog = DialogSystem.instance.UpdateDialog(npcDialogList
                    .Where(dialogDB => dialogDB.DialogID == _saveDialogID).ToList(), npcAnimator, npcVirtualCameraObj));
    ...
    {
        var findCount = npcDialogList.Find(entity => entity.DialogID == _saveDialogID + 1).DialogID;
        print($"{findCount}");
        
        // 저장된 기본 다이얼로그 ID값을 1증가시켜 다음 다이얼로그ID로 변경해준다.(퀘스트 다이얼로그 제외)
        if(findCount != 0 && _saveDialogID % dialogID < 90)
            _saveDialogID++;
    }
}
