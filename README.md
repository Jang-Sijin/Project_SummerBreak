...
```
- 이외 작업 내용이 너무 많거나 문제 발생 가능성이 적다면 간략하게 기입하셔도 무방할 것 같습니다.
- **차후 문제가 발생하면 대처 방안을 readme에 추가하는 방향으로 갑시다.**
...
```
  ** 같은 부분을 수정하다가 타인이 작업한 내용이 유실될 가능성이 있음 **
  때문에 테스트를 진행 후 검증된 데이터만 Main Scene에 적용하는 방식으로 진행하는 것이 좋습니다.
```
원래는 Branch를 생성해서 작업을 진행 후 Merge 하는 방법이 Best → **Branch 사용이 익숙해질 때까지는 위의 내용으로 진행합시다.**
